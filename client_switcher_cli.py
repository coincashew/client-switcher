...
'TimeoutStopSec=900',
'Environment=RUST_LOG=info',
f'ExecStart=/usr/local/bin/erigon --datadir /var/lib/erigon --chain {eth_network} --port {EL_P2P_PORT} --torrent.port 42069 --http.port {EL_RPC_PORT} --maxpeers {EL_MAX_PEER_COUNT} --http.api web3,eth,net,engine --metrics --pprof --prune.mode=minimal --externalcl --authrpc.jwtsecret={JWTSECRET_PATH}',
'',
'[Install]',
'WantedBy=multi-user.target',
    ]
